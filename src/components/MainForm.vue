...
<!--      <a class="lang" href="">EN</a>-->
    ...
    <div class="results" v-if="isCalculated">
      <dive-display v-if="display.firstDive.stops.GPS !== '!'" :dive="display.firstDive" />
      <dive-display v-if="display.hasSurfaceInterval && display.firstDive.stops.GPS !== '!'" :dive="display.secondDive" />
    </div>
    <div v-if="display.firstDive.stops.GPS" class="variables">
      <div>
        <h3>Première plongée</h3>
        <p>Profondeur majorée pour {{ display.firstDive.depth }}m : {{ display.firstDive.refDepth }}m</p>
        <p>Temps de plongée majoré pour {{ display.firstDive.duration }}min : {{ display.firstDive.refDuration }}min</p>
        <p>Durée totale de remontée : {{ display.firstDive.stops.DTR }}min</p>
        <p>GPS : {{ display.firstDive.stops.GPS }}</p>
      </div>
    </div>
    <div v-if="display.hasSurfaceInterval && display.firstDive.stops.GPS !== '!'" class="variables">
      <div>
        <h3>Interval de surface</h3>
        <p>Interval de surface majoré pour {{ display.interval }}min : {{ display.refInterval }}min</p>
      </div>
    </div>
    <div v-if="display.hasSurfaceInterval && display.firstDive.stops.GPS !== '!'" class="variables">
      <div>
        <h3>Deuxième plongée</h3>
        <p>Profondeur majorée pour {{ display.secondDive.depth }}m : {{ display.secondDive.refDepth }}m</p>
        <p>Temps de plongée majoré pour {{ display.secondDive.duration }}min : {{ display.secondDive.refDuration }}min</p>
        <p>Durée totale de remontée : {{ display.secondDive.stops.DTR }}min</p>
        <p>GPS : {{ display.secondDive.stops.GPS }}</p>
      </div>
    </div>
    <dive-chart v-model="display" />
